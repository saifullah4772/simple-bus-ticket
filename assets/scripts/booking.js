const tdCollection = document.getElementsByClassName("seat_number");
console.log(tdCollection);
const collectionArray = Array.from(tdCollection);
const setSeatValue = (seat) => {
  console.log(seat);
  document.getElementById("seat_selected").value = seat;
};
collectionArray.map((val) => {
  const seatNo = val.getAttribute("data-seatNo");
